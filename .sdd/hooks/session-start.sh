...
hook_finish() {
  code=$?
  if [ "$code" -eq 0 ]; then printf '{"hook":"SessionStart","pass":true,"reason":"context inspected"}\n'; fi
}
...
if missing:
    ...
    print(f'Deliverable: {len(checks) - len(missing)}/{len(checks)} present, missing: {missing_text}')
else:
