...
hook_allow() {
  printf '{"hook":"Stop","pass":true,"reason":"%s"}\n' "$1"
  exit 0
}

hook_block() {
  printf '{"hook":"Stop","pass":false,"reason":"%s"}\n' "$1" >&2
  exit 2
}
...
if [ "$GATE_STATUS" = "failed" ]; then
  hook_block "stage=${CURRENT_STAGE} gate status=failed"
fi

if [ "$GATE_STATUS" = "pending" ] && [ "$CURRENT_STAGE" != "grill" ]; then
  hook_block "stage=${CURRENT_STAGE} gate status=pending"
fi
...
if [ -z "$DELIVERABLE_RESULT" ]; then
  hook_block "deliverable check unavailable"
fi
...
if [ "$REQUIRED_COUNT" -gt 0 ]; then
  hook_block "stage=${CURRENT_STAGE} required deliverables missing=${REQUIRED_COUNT}"
fi
...
hook_allow "deliverables verified; expected gaps=${EXPECTED_COUNT}"
