...
2. 进入全部台账，底部总数应为 93。搜索 `PC-080`，结果应包含新闻资讯；点击详情，地址变为 `/operations/requirements/pc-080`，抽屉显示独立 source SHA 与逐记录 evidence hash。
3. 清空搜索，选择任一业务域，表格只显示该域。刷新域路由后筛选仍由 URL 恢复。
4. 选择 UNCONFIRMED 状态，总数应为 24。共享台账只显示状态、来源、详情和导航，没有 mutation surface。
5. 返回已确认范围，确认同样只有详情与专用能力导航。通用 mutation 不属于共享台账职责。
