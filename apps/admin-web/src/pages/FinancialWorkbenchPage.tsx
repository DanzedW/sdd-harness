...
import { FINANCE_CAPABILITIES } from "../operations/domainCatalog";

export function FinancialWorkbenchPage() {
  const [rows, setRows] = useState<OperationRequirement[]>([]);
  useEffect(() => { void operationsService.query({ pageSize: 200 }).then(({ items }) => setRows(items.filter((item) => getWorkbenchKind(item) === "finance"))); }, []);
  return <Space direction="vertical" size={16} style={{ width: "100%" }}>
    ...
    <Card title="现有专用能力（聚合页不替代专页）"><Table rowKey="route" pagination={false} dataSource={FINANCE_CAPABILITIES} columns={[
      { title: "能力", dataIndex: "name", width: 140 },
      { title: "专用入口", width: 160, render: (_, item) => <Button type="link" href={item.route}>{item.route}</Button> },
      { title: "页面证据", dataIndex: "page" },
      { title: "Service 证据", dataIndex: "service" },
      { title: "测试证据", dataIndex: "test" },
    ]} /></Card>
